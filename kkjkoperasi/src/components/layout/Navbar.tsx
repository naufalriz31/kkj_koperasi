...
export const Navbar = () => {
    ...
    // Jika tidak ada user, Navbar tidak muncul
    if (!user) return null;

    // [KUNCI PERBAIKAN]: Jika role adalah Admin, jangan tampilkan Navbar global (Member) ini.
    // Admin sudah memiliki Header sendiri di AdminDashboard.tsx.
                ...
                <div className="flex justify-between h-16 items-center">
                    ...
                    <div className="flex-shrink-0 flex items-center cursor-pointer">
                        <Link to="/" className="flex items-center gap-3">
                            ...
                            <div className="hidden md:block leading-tight">
                                <span className="block font-extrabold text-[#136f42] text-sm uppercase tracking-wide">Koperasi KKJ</span>
                                <span className="block text-[10px] text-gray-500 font-medium tracking-wider">MITRA SEJAHTERA</span>
                            </div>
                        </Link>
                    </div>

                    {/* 2. MENU TENGAH (MEMBER ONLY) */}
                    <div className="hidden md:flex flex-1 justify-center items-center gap-8">
                        {memberLinks.map((link) => (
                            <Link
                                ...
                                className={`text-sm font-medium transition-colors border-b-2 py-5 ${isActive(link.path)
                                    ? 'border-[#136f42] text-[#136f42] font-bold'
                        ...
                        <div className="relative group">
                            <button className="flex items-center gap-1 text-gray-700 hover:text-[#136f42] font-medium py-2">
                            ...
                            <div className="absolute top-full left-0 w-48 bg-white shadow-xl rounded-xl border border-gray-100 p-2 hidden group-hover:block z-50 transform transition-all duration-200 mt-1">
                                ...
                                <Link to="/transaksi" className="block px-4 py-2 text-sm font-bold text-[#136f42] hover:bg-blue-50 rounded-lg">Semua Menu</Link>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        <Link to="/notifikasi" className="relative p-2 text-gray-400 hover:text-[#136f42] transition-colors rounded-full hover:bg-gray-100">
                ...
                <div className="md:hidden bg-white border-t border-gray-100 p-4 shadow-lg absolute w-full left-0 top-16 h-screen overflow-y-auto pb-32 animate-in slide-in-from-top-5 z-40">
                    <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl mb-6 border border-gray-100 shadow-sm">
                        ...
                        <div>
                            ...
                            <Link to="/profil" onClick={() => setIsOpen(false)} className="text-sm text-[#136f42] font-bold hover:underline">Edit Profil</Link>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        {memberLinks.map((link) => (
                            <Link
                                ...
                                className={`block px-4 py-3 rounded-xl text-base font-bold transition-all ${isActive(link.path) ? 'bg-green-50 text-[#136f42] border border-green-100' : 'text-gray-600 bg-white border border-gray-100 hover:bg-gray-50'
                                    }`}
                            >
                                {link.label}
                            </Link>
                        ...
                        <Link
                            to="/transaksi"
                            onClick={() => setIsOpen(false)}
                            className={`block px-4 py-3 rounded-xl text-base font-bold flex justify-between items-center transition-all ${location.pathname.includes('/transaksi') ? 'bg-green-50 text-[#136f42] border border-green-100' : 'text-gray-600 bg-white border border-gray-100 hover:bg-gray-50'
                                }`}
                        >
