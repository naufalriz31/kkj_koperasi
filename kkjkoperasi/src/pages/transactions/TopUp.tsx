...
import API from '../../api/api'; // Pastikan baseURL: 'http://127.0.0.1:8000/api'
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24 font-sans">
            ...
            <div className="max-w-xl mx-auto p-4 space-y-8">
                <div className="space-y-3">
                    ...
                    {bankAccounts.map((bank, idx) => (
                        <div key={idx} className="bg-white p-5 rounded-2xl border border-green-100 shadow-sm flex justify-between items-center group hover:border-[#136f42] transition-colors">
                            ...
                            <button type="button" onClick={() => handleCopy(bank.number)} className="p-2 text-gray-400 hover:text-[#136f42] hover:bg-green-50 rounded-xl transition-all active:scale-95">
                                <Copy size={20} />
                            </button>
                        </div>
                    ))}
                ...
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-green-100 shadow-sm space-y-6">
                    <div>
