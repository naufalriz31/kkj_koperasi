...
interface KabarData {
    id: number | string;
    title: string;
    content: string;    // Perbaikan: Sesuai nama kolom di MySQL
    category: string;   // Perbaikan: Sesuai nama kolom di MySQL
    ...
    useEffect(() => {
        const fetchKabar = async () => {
            ...
            try {
                ...
                toast.error("Kabar tidak ditemukan atau sudah dihapus");
                navigate('/');
            } finally {
                setLoading(false);
            }
        ...
        if (navigator.share && kabar) {
            try {
                await navigator.share({
                    title: kabar.title,
                    text: kabar.content.substring(0, 100) + '...',
            ...
            toast.success("Link kabar disalin!");
        }
    };
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="flex flex-col items-center gap-4">
                    <div className="animate-spin rounded-full h-12 w-12 border-4 border-[#136f42] border-t-transparent"></div>
                    <p className="text-sm font-bold text-gray-400">Memuat Kabar...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex-1 relative z-10 text-white flex flex-col justify-center h-full w-full order-2 md:order-1">
                        <div className="flex flex-wrap items-center gap-3 mb-5">
                            <span className={cn(
                                ...
                                kabar.category === 'INFO' ? 'bg-rose-600' : 
                                kabar.category === 'PROMO' ? 'bg-emerald-500 text-white' : 'bg-[#aeea00] text-[#136f42]'
                            )}>
                                {kabar.category}
                            </span>
                            ...
                            <div className="w-full h-full rounded-2xl overflow-hidden bg-gray-50 relative">
                                {kabar.image_url ? (
                                    <img 
                                        ...
                                        onError={(e) => {
                                            (e.target as HTMLImageElement).src = 'https://placehold.co/600x400?text=Kabar+KKJ';
                                        }}
                                    />
                                ) : (
